...
const COMPILE_EXEC = 'source dist/ui/prompt.func.pkg.sh'
...
PID=$!; sleep 0.1; kill $PID`, execOpts)
  ...
  const result = shell.exec(`${STRICT}; ${COMPILE_EXEC}; timeout() { perl -e 'alarm shift; exec @ARGV' "$@"; }; unset FOO; export -f require-answer; export -f setSimpleOptions; setSimpleOptions() { :; }; timeout 1 bash -c require-answer "prompt: " FOO`, execOpts)
  ...
  test(`displays prompt`, () => {
    ...
    const result = shell.exec(`${STRICT}; ${COMPILE_EXEC}; timeout() { perl -e 'alarm shift; exec @ARGV' "$@"; }; unset FOO; export -f require-answer; export -f setSimpleOptions; setSimpleOptions() { :; }; bash -c require-answer "prompt: " FOO &
PID=$!; sleep 0.1; kill $PID`, execOpts)
...
unset FOO; require-answer "prompt: " FOO <<< ${input}; echo "FOO: $FOO"`, execOpts) */
  ...
  test(`does not ask any questions when var already set`, () => {
    ...
    result = shell.exec(`${STRICT}; ${COMPILE_EXEC}; FOO=foo; require-answer "prompt: " FOO <<< bar; echo "FOO: $FOO"`, execOpts)
...
describe('yes-no', () => {
  ...
  test.only(`displays prompt as expected`, () => {
    ...
    myOpts.silent = false
    // const result = shell.exec(`${STRICT}; ${COMPILE_EXEC}; yes-no 'This is the prompt: '`, myOpts)
    const result = shell.exec(`${STRICT}; ${COMPILE_EXEC}; yes-no 'This is the prompt: ' <<< y`, execOpts)
