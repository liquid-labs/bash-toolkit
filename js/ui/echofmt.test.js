...
describe('echofmt', () => {
  test('echo hi', () => {
    const result = shell.exec(`set -e; ${COMPILE_EXEC}; echofmt hi`, execOpts)
    const expectedOut = expect.stringMatching(/^.*hi.*\n$/)
    assertMatchNoError(result, expectedOut)
  })

  test('echo "hi\nbye"', () => {
    const result = shell.exec(`set -e; ${COMPILE_EXEC}; echofmt "hi\nbye"`, execOpts)
    const expectedOut = expect.stringMatching(/^.*hi\nbye.*\n$/)
    assertMatchNoError(result, expectedOut)
  })
  ...
  test('folds longer lines', () => {
    ...
    const result = shell.exec(`set -e; ${COMPILE_EXEC}; echofmt ${ax80} abc`, execOpts)
  ...
  describe('when ECHO_QUIET=true', () => {
    test('supresses regular messages', () => {
      const result = shell.exec(`set -e; ${COMPILE_EXEC}; ECHO_QUIET=true echofmt hi`, execOpts)
      assertMatchNoError(result, '')
    })

    test('supresses info messages', () => {
      const result = shell.exec(`set -e; ${COMPILE_EXEC}; ECHO_QUIET=true echofmt --info hi`, execOpts)
      assertMatchNoError(result, '')
    })

    test('echos warning messages', () => {
      const result = shell.exec(`set -e; ${COMPILE_EXEC}; ECHO_QUIET=true echofmt --warn hi`, execOpts)
      checkQuiteEchos(result)
    })

    test('echoes error messages', () => {
      const result = shell.exec(`set -e; ${COMPILE_EXEC}; ECHO_QUIET=true echofmt --error hi`, execOpts)
      checkQuiteEchos(result)
    })
  })
  ...
  describe('when ECHO_SILENT=true', () => {
    test('supresses regular messages', () => {
      const result = shell.exec(`set -e; ${COMPILE_EXEC}; ECHO_SILENT=true echofmt hi`, execOpts)
      assertMatchNoError(result, '')
    })

    test('supresses info messages', () => {
      const result = shell.exec(`set -e; ${COMPILE_EXEC}; ECHO_SILENT=true echofmt --info hi`, execOpts)
      assertMatchNoError(result, '')
    })

    test('supresses warning messages', () => {
      const result = shell.exec(`set -e; ${COMPILE_EXEC}; ECHO_SILENT=true echofmt --warn hi`, execOpts)
      assertMatchNoError(result, '')
    })

    test('echoes error messages', () => {
      const result = shell.exec(`set -e; ${COMPILE_EXEC}; ECHO_SILENT=true echofmt --error hi`, execOpts)
      checkQuiteEchos(result)
    })
  })
})
